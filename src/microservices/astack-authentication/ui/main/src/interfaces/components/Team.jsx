...
export default function Team({Team}) {
    ...
    useEffect(
        () => {
            ...
            const onLoad = () => {
                setPlayerList(Team);
                let player = Team[0];
                let teamName = addStToChurch(player['team_church']).toUpperCase() + " " + returnTeamLevelAsString(player["team_level"]).toUpperCase() + " " + player["team_cap_name"].toUpperCase();
    ...
    /**
     * Add St. to the front of church name if necessary
     */
     function addStToChurch (teamChurch) {
        let teamNames = {
        "luke":1,
        "giles":1,
        "celestine":1,
        ...
        if (teamNames[teamChurch] == 1){
            return "st. " + teamChurch
        }
        return teamChurch;
